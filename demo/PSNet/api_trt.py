...
# inference pipeline
def infer(engine, ndarray, device):
    """
    :param engine:
    :param ndarray: HWC
    :return:
    ...
    input_image = preprocess(image_pil)  # pil to [c, h, w]
    ...
    circle, mask = postprocess(outputs, (image_h, image_w), device)
    return circle, mask
...
from .api import before_input, after_output
import cv2
# with load_engine(engine_file) as engine:
#     infer(engine, image_path)
def detect_image_iris_trt(ndarray, engine, device, vis=False):
    """
    ...
    """
    image_h, image_w = ndarray.shape[:2]
    image = before_input(ndarray)  # deep copy, [h, w, c]
    af_size = image.shape[:2]  # is too small, resize to bigger, [h, w].

    detected_circle, predicted_mask = infer(engine, image, device)
    ...
    if vis:
        ...
        # show.show()
        circle, mask = after_output(detected_circle, predicted_mask, af_size, (image_h, image_w))
